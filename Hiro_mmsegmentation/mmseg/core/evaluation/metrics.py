...
#         label[label == 254] = 255
    ...
    if multi_label==True:
        ...
        area_pred_label = pred_label.sum(dim=(0,1))
        area_label = label.sum(dim=(0,1))
        correct = pred_label*label
        area_intersect = correct.sum(dim=(0,1))
...
def total_area_to_metrics(total_area_intersect,
    ...
    if isinstance(metrics, str):
        metrics = [metrics]
    allowed_mean_metrics = ['mIoU', 'mDice', 'mFscore', 'PR']
    allowed_image_metrics = ['imIoU', 'imDice', 'imFscore', 'PR']
    ...
    acc = total_area_intersect / (total_area_label+1e-7)
    ret_metrics['Acc'] = acc
...
#             ret_metrics['Fscore'] = f_value
            ...
            iou = (image_area_intersect / (image_area_union + 1e-7) + (image_area_union == 0).long()).mean(0)
            ret_metrics['IoU'] = iou
        elif metric == 'imDice':
            dice = (2 * image_area_intersect / (
                image_area_pred_label + image_area_label + 1e-7) + (image_area_union == 0).long()).mean(0)
            ret_metrics['iDice'] = dice
        elif metric == 'imFscore':
            precision = (image_area_intersect / (image_area_pred_label + 1e-7) + (image_area_union == 0).long()).mean(0)
