...
def py_sigmoid_focal_loss(pred,
    ...
    pred_sigmoid = F.logsigmoid(pred).exp()
    target = target.type_as(pred)
    fp = pred_sigmoid * (1 - target)
    fn = (1 - pred_sigmoid) * target
#     fp[:,0], fn[:,0] = 1.6 * fp[:,0], 0.6 * fn[:,0]
#     fp[:,1], fn[:,1] = 1.2 * fp[:,1], 1.8 * fn[:,1]
#     fp[:,2], fn[:,2] = 1.1 * fp[:,2], 1.1 * fn[:,2]
    one_minus_pt = fp + fn
    ...
    def forward(self,
        ...
        if self.use_sigmoid:
            ...
            if torch.cuda.is_available() and pred.is_cuda:
                ...
                calculate_loss_func = py_sigmoid_focal_loss#sigmoid_focal_loss
            else:
                one_hot_target = None
                if target.dim() == 1:
