...
class CustomDataset(Dataset):
    ...
    def pre_eval(self, preds, indices):
        ...
        for pred, index in zip(preds, indices):
            ...
            out = intersect_and_union(pred, seg_map, len(self.CLASSES),
                                    self.ignore_index, self.label_map,
                                    self.reduce_zero_label, self.multi_label)
            pre_eval_results.append(out)
    ...
    def evaluate(self,
        ...
        if isinstance(metric, str):
            metric = [metric]
        allowed_metrics = ['mIoU', 'mDice', 'mFscore', 'imDice', 'imIoU', 'imFscore', 'PR']
